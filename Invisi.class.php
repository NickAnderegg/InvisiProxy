...
/*
   +-----------------+------------------------------------------------------------+
   |  Class          | Invisi													  |
   |  Author         | Nick Anderegg (includes original code by ultimategamer00)  |
   |  Last Modified  | 5:45 PM 2/20/2011                                          |
   +-----------------+------------------------------------------------------------+
...
class Invisi
{
   ...
   (
    ...
    function Invisi($config, $flags = 'previous')
    {
        $this->version    = '0.1';
    ...
    function set_request_headers()
    {
        ...
        if ($this->request_method == 'POST')
        {
            ...
            {
                $this->data_boundary = 'Invisi-' . md5(uniqid(rand(), true));
    ...
    {
		// "$tabulate" means "make into a table."
        $output   = '';
        $comments = array();
